	...
	if(connectionServiceFn($username,$password,$plName)){
		...
		$("#btn_submit_edit_"+$appName).on('click',function(){
			...
			if($name != ''){
				...
				$data = {case_type:$name,is_active:$is_active};
			...
			$temp = '<tr>'
					+'<td style="vertical-align: middle;">'+this.case_type+'</td>'
						...
						+'<i class="fa fa-cog font-gear popover-edit-del"'
							...
							+'data-content="'
								+'<button class=\'btn btn-warning btn-xs btn-gear edit\' data-target=#modalEdit data-toggle=\'modal\' data-id=\''+this.case_type_id+'\'>Edit</button>'
								+'<button class=\'btn btn-danger btn-xs btn-gear del\' data-target=#confrimModal data-toggle=\'modal\' data-id=\''+this.case_type_id+'\' style=\'margin-left: 15px\'>Delete</button>">'
						+'</i></td>'
					+'</tr>';
			$html+= $temp;
		});
