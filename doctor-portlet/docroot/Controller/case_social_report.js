	 ...
	 if(username!="" && username!=null & username!=[] && username!=undefined ) {
		 if(connectionServiceFn(username,password,plid)==true) {
			...
			function DropDownSocial() {
				$.ajax({
					url:restfulURL+"/"+serviceName+"/report/list_social",
					type:"get",
					dataType:"json",
					async:true,
					headers:{Authorization:"Bearer "+tokenID.token},
					success:function(data){
						var htmlOption="";
						htmlOption+="<option value='All'>All</option>";
						$.each(data,function(index,indexEntry) {
							htmlOption+="<option value="+indexEntry['social_media_id']+">"+indexEntry['social_media_name']+"</option>";
						});
						$("#social").html(htmlOption);
					}
				});
			}
			...
			$('.datepicker').datepicker({
	 		...
	 		$("#btn_search").click(function() {
	 			var parameter = {
	 					...
	 					param_social_id: $("#social").val()
		 		}
	 			
	 			var url_report_jasper = restfulURL+"/"+serviceName+"/report/api_report?template_name=report-10&template_format=pdf&used_connection=1&inline=1&data="+JSON.stringify(parameter);
